...
智能整理功能：自动识别多个物体属性并按规则排序整理，比如：
  - 按颜色深浅排列：organize_objects("把桌上的水果按颜色深浅从深到浅排列")
  - 把散落物品叠整齐：organize_objects("把散落的卡片叠整齐，放在桌子右侧")
  - 按大小排序：organize_objects("把积木按从小到大排列")
  这个函数会自动完成：检测所有物体、分析属性（颜色、大小、是否易碎、是否不规则形状）、生成抓取序列、轻柔操作易损物品、生成整理前后对比图像。
